...
function appendUserMsg(json) {
	...
	if (json.type == 1) {
		...
		//			json.msg = '';
		//			html += transEmo(json.msg);
		//		} else if (json.msg == 'orderProcess') {
		//			json.msg = '点击‘个人中心》我的订单/拼团订单’';
		//			html += transEmo(json.msg);
		//		} else if (json.msg == 'cancelOrder') {
		//			json.msg = '点击‘个人中心》我的订单/拼团订单’,找到需要取消的订单';
		//			html += transEmo(json.msg);
		//		} else if (json.msg == 'returnOrder') {
		//			json.msg = '点击‘个人中心》我的订单/拼团订单’,找到需要退换货的订单';
		//
		//			html += transEmo(json.msg);
		//		} else {
		//
		...
		if (!isNotNull(chatImg)) {
			html += '<div class="with-img"><img src="../img/logo.png" /></div>';
		} else {
		...
		//		}
		//		html += '</div>';
		//		html += '<div class="newlioatian">';
		//		html += '<div calss="newliaotianTitle">亲，下午好，有什么可以帮助你的吗？</div>';
		//		html += '<div class="newliaotianText">';
		//		html += '<ul>';
		//		var str = '';
		//		str = "下单流程";
		//		html += '<li tapmode="" onclick="sendMsg(1,\'' + str + '\');">下单流程</li>';
		//		str = "我要取消订单";
		//		html += '<li tapmode="" onclick="sendMsg(1,\'' + str + '\');">我要取消订单</li>';
		//		str = "申请退换货失败";
		//		html += '<li tapmode="" onclick="sendMsg(1,\'' + str + '\');">申请退换货失败</li>';
		//		html += '</ul>';
		//		html += '</div>';
		//		html += '</div>';
		//		html += '</div>';
	...
	if (systemType == "android") {
		document.body.scrollTop += $("#content_send_" + json.time).height();
	} else {
		document.body.scrollTop = document.body.scrollHeight;
	}
	//document.body.scrollTop = document.body.scrollHeight;
...
function appendHistoryMsg(json) {
	var value = config.serviceDefault;
	...
	if (!isNotNull(chatImg)) {
		html2 += '<img src="../img/verify03.png" />';
	} else {
		html2 += '<img src="' + chatImg + '" />';
	}
	html2 += '</div>';
	html2 += '<div class="with-text"  tapmode=""  onclick="setClipboardChat(\'' + value + '\')">';
	html2 += transEmo(value);
	html2 += '</div>';
	html2 += '</div>';
	html2 += '</div>';
	...
	if (isNotNull(jsonData.order)) {
		html2 += '<div class="dingnewnew">';
		html2 += '<div class="dingnewnew-top">';
		html2 += '<div class="dingnewnewimgs"><img src="../img/goodsdetailimg.png" id="product_yl_' + jsonData.order.id + '"/></div>';
		html2 += '<div class="dingnewnew-title">';
		html2 += '<div class="dingnewnew-title">' + jsonData.order.orderDetails[0].productName + '</div>';

		//html2 += '<div class="dingltextbox">￥' + formatterNumber(jsonData.order.orderDetails[0].price) + '</div>';
		html2 += '<div class="dingls">';
		html2 += '<div class="dingl-left"><span>';
		if (jsonData.order.status == 1) {
			if (jsonData.order.groupStatus == 0) {//待分享
				html2 += '待分享';
			} else {
				html2 += '待发货';
			}
		} else if (jsonData.order.status == 2) {
			html2 += '待收货';
		} else if (jsonData.order.status == 3) {
			html2 += '待评价';
		} else if (jsonData.order.status == 4) {
			html2 += '已完成';
		} else if (jsonData.order.status == 1 && jsonData.order.groupStatus == 0) {

		} else if (jsonData.order.status == -1) {
			if (jsonData.order.groupStatus == 2) {//未成团
				html2 += '拼单失败';
			} else if (jsonData.order.groupStatus == 3) {//已退款
		...
		html2 += '</span></div>';
		html2 += '<div class="ding-buttons"><button tapmode="" onclick="consultationOrder(\'' + jsonData.order.id + '\');">咨询该订单</button></div>';
		html2 += '</div>';
		html2 += '</div>';
		html2 += '</div>';
		html2 += '</div>';
	...
	api.parseTapmode();
	if (isNotNull(jsonData.order)) {
		cacheImage("product_yl_" + jsonData.order.id, jsonData.order.orderDetails[0].productImg2);
	}
	hideProgress();
	if (systemType == "android") {
	...
	UIChatBox.open({
		...
		if (ret) {
			...
			if (ret.eventType == 'send') {
				if (ret.msg.length > 0 && isNotNull(ret.msg)) {
					sendMsg(1, ret.msg);
				} else {
					toast("发送内容不能为空");
					return false;
			...
			if (ret.code == 1) {
				if (isNotNull(ret.orderList)) {
					for (var i = 0; i < ret.orderList.length; i++) {
						ret.orderList[i].isShow = 1;
					}
					allOrder = ret.orderList;
					UIChatBox.hide();
...
function selectOrder(orderList) {
	var html = '';
	for (var i = 0; i < orderList.length; i++) {
		if (orderList[i].isShow == 1) {
			html += '<div class="dingnewnew">';
			html += '<div class="dingnewnew-top">';
			html += '<div class="dingnewnewimgs"><img src="../img/goodsdetailimg.png" id="orderImg_' + orderList[i].id + '"/></div>';
			html += '<div class="dingnewnew-title">';
			html += '<div class="dingnewnew-title">' + orderList[i].orderDetails[0].productName + '</div>';

			html += '<div class="dingltextbox">￥' + formatterNumber(orderList[i].orderDetails[0].price) + '</div>';
			html += '<div class="dingls">';
			html += '<div class="dingl-left"><span>';
			if (orderList[i].status == 1) {
				if (orderList[i].groupStatus == 0) {//待分享
					html += '待分享';
				} else {
					html += '待发货';
				}
				...
				html += '已完成';
			} else if (orderList[i].status == 1 && orderList[i].groupStatus == 0) {

			} else if (orderList[i].status == -1) {
				if (orderList[i].groupStatus == 2) {//未成团
					html += '拼单失败';
				} else if (orderList[i].groupStatus == 3) {//已退款
			...
			html += '</span></div>';
			html += '<div class="ding-buttons"><button tapmode="" onclick="sendOrder(\'' + orderList[i].id + '\');">发送</button></div>';
			html += '</div>';
			html += '</div>';
			html += '</div>';
			html += '</div>';
	...
	for (var i = 0; i < orderList.length; i++) {
		cacheImage("orderImg_" + orderList[i].id, orderList[i].orderDetails[0].productImg2);
	}
...
function searchList() {
	var selectStr = $("#searchText").val();
	var orderArr = Array();
	var t = 0;
	for (var i = 0; i < allOrder.length; i++) {
		var goodStr = allOrder[i].orderDetails[0].productName;
		if (goodStr.indexOf(selectStr) != -1) {
			orderArr[t] = allOrder[i];
			t++;
		}
	}
	$("#searchText").blur();
	selectOrder(orderArr);
}
