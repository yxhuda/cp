...
function openSocket() {
	...
	webSocket.addEventListener(function(ret, err) {
		if (isNotNull(ret)) {
			...
			} else if (ret.code == 3 || ret.code == 4 || ret.code == 5) {
				connectSocket();
			}
		} else {
			connectSocket();
...
function appendSelfMsg(json) {
	...
	html += '<div class="borderlink"></div><div class="with-time">' + formatChatTime(parseInt(json.time) * 1000) + '</div>';
	if (json.type == 1) {
		...
		html += '</div>';
		html += '<div class="with-text-right" id="selfMsg' + json.time + '" tapmode="">';
		...
		html += '<div class="vo-text-right"  tapmode=""  onclick="play(\'' + json.msg + '\',\'right_b_' + json.time + '\')"><img src="../img/right_b.png" id="right_b_' + json.time + '"  data-icon="../img/right_b.png" data-iconGif="../img/right_b1.gif"/> &nbsp;' + json.dur_time + '"</div>';
		if (!isNotNull(user.header)) {
		...
		html += '<div class="consultingbox" tapmode="" onclick="getGoods(\'' + json.product[0].id + '\',\'' + json.product[0].is_groupbuy + '\');">';
...
function appendUserMsg(json) {
	...
	if (json.type == 1) {
		...
		html += '</div>';
		html += '<div class="with-text" id="UserMsg' + json.time + '" tapmode="" >';
		html += transEmo(json.msg);
		html += '</div>';
		html += '</div>';
	} else if (json.type == 2) {
		...
		html += '</div>';
		html += '<div class="with-text noborders">';
		html += '<img src="' + json.msg + '" style="width: 100px;height: 100px;" tapmode="" onclick="browsePictures(\'' + json.msg + '\');"/>';
		html += '</div>';
		html += '</div>';
	} else if (json.type == 3) {
		...
		html += '<div class="vo-text-left"  tapmode=""  onclick="play(\'' + json.msg + '\',\'left_b_m_' + json.time + '\')"><img src="../img/left_b.png" id="left_b_m_' + json.time + '" data-icon="../img/left_b.png" data-iconGif="../img/left_b1.gif"/> &nbsp;' + json.dur_time + '"</div>';
		html += '</div>';
	} else if (json.type == 4) {
		html += '<div class="consulting">';
		html += '<div class="consultingbox"  tapmode="" onclick="getGoods(\'' + json.product[0].id + '\',\'' + json.product[0].is_groupbuy + '\');">';
...
function appendHistoryMsg(json) {
	...
	if (json.status == 1) {
		...
		for (var i = 0; i < json.chatHistory.length; i++) {
			...
			if (json.chatHistory[i].send_user == user.id) {//发送人是登录用户
				...
				html += '<div class="borderlink"></div><div class="with-time">' + formatChatTime(parseInt(json.chatHistory[i].send_time) * 1000) + '</div>';
				if (json.chatHistory[i].msg_type == 1) {
					...
					html += '</div>';
					html += '<div class="with-text-right" id="selfMsg' + json.chatHistory[i].send_time + '" tapmode="" >';
					...
					html += '</div>';
					html += '<div class="with-text-right noborders">';
					...
					html += '<div class="vo-right">';
					html += '<div class="vo-text-right"  tapmode=""  onclick="play(\'' + json.chatHistory[i].contents + '\',\'right_b_' + json.chatHistory[i].send_time + '\')"><img src="../img/right_b.png" id="right_b_' + json.chatHistory[i].send_time + '"  data-icon="../img/right_b.png" data-iconGif="../img/right_b1.gif"/> &nbsp;' + json.chatHistory[i].dur_time + '"</div>';
					if (!isNotNull(user.header)) {
						html += '<div class="with-img-right"><img src="../img/verify03.png" /></div>';
					} else {
						html += '<div class="with-img-right"><img src="' + user.header + '" /></div>';
					...
					html += '<div class="content-box">';
					html += '<div class="consulting">';
					html += '<div class="consultingbox" tapmode="" onclick="getGoods(\'' + json.chatHistory[i].product[0].id + '\',\'' + json.chatHistory[i].product[0].is_groupbuy + '\');">';
				...
				if (json.chatHistory[i].msg_type == 1) {
					...
					html += '</div>';
					html += '<div class="with-text" id="UserMsg' + json.chatHistory[i].send_time + '" tapmode="">';
					html += transEmo(json.chatHistory[i].contents);
					html += '</div>';
					html += '</div>';
				} else if (json.chatHistory[i].msg_type == 2) {
					...
					html += '<div class="vo-text-left"  tapmode=""  onclick="play(\'' + json.chatHistory[i].contents + '\',\'left_b_m_' + json.chatHistory[i].send_time + '\')"><img src="../img/left_b.png" id="left_b_m_' + json.chatHistory[i].send_time + '" data-icon="../img/left_b.png" data-iconGif="../img/left_b1.gif"/> &nbsp;' + json.chatHistory[i].dur_time + '"</div>';
					html += '</div>';
				} else if (json.chatHistory[i].msg_type == 4) {
					html += '<div class="content-box">';
	...
	if (isNotNull(jsonData.product)) {
		...
		html2 += '<div class="dingnewnewimg"><img src="img/groupBullet.png" id="product_yl_' + jsonData.product.id + '"/></div>';
		html2 += '<div class="dingnewnew-title-box">';
		html2 += '<div class="dingnewnew-title">' + jsonData.product.productName + '</div>';
		html2 += '<div class="dingl">';
		...
		html2 += '<div class="ding-button"><button tapmode="" onclick="consultationGoods(\'' + jsonData.product.id + '\');">咨询该商品</button></div>';
		html2 += '</div>';
		html2 += '</div>';
		html2 += '</div>';
		html2 += '<div class="dingtext">';
...
function formatChatTime(time) {
	...
	var morning = new Date(new Date().setHours(0, 0, 0, 0)) / 1000;    // 获取当天 0 点的时间戳
...
//跳转商品详情
function getGoods(id, is_groupbuy) {
	tag = true;
	if (is_groupbuy == 1) {
