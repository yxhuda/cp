	...
	ajaxRequest(url, bodyParam, function(ret, err) {
		...
		if (err) {
			...
			if (ret.code == 1) {
				...
				$("#activity").empty();
				api.refreshHeaderLoadDone();
				getRecommendProduct(3, 'activity');
			}
		}
	});
		...
		case 1:
			var a = 0;
				for (var j = 0; j < data.orderDetails.length; j++) {
					if (parseInt(data.orderDetails[j].refundStatus) == 8) {
						a++
					}
				}
			if (a == data.orderDetails.length) {
				title = '已退款';
			}
			else {
				title = '待发货';
...
//			title = '待评价';
	...
	for (var i = 0;i<data.orderDetails.length;i++) {
		html += '<div tapmode="" onclick="goOrderDetails(\'' + data.orderDetails[i].orderId + '\');">';
		html += '<div class="confirmorderBox">';
		html += '<div class="confirmorderBox-img"><img src="../img/goodsbanner.png" id="productImg_' + data.id + '_' + data.orderDetails[i].id + '"/></div>';
		html += '<div class="confirmorderBox-text">';
		html += '<div class="confirmorderBox-title">' + data.orderDetails[i].productName + '</div>';
		if (isNotNull(data.orderDetails[i].specDesc)) {
			html += '<div class="confirmorderBox-name">规格：' + data.orderDetails[i].specDesc + '</div>';
		} else {
			html += '<div class="confirmorderBox-name">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>';
		}
		
		html += '<div class="myorder-price"><span>￥ ' + formatterNumber(data.orderDetails[i].price) + '</span><span>x' + data.orderDetails[i].num + '</span></div>';
		var title2 = '';
		switch(parseInt(data.orderDetails[i].refundStatus)) {
			case 1:
				title2 = '审核中';
				break;
			case 5:
		...
		html += '<div  style="margin-bottom:0;height:30px;width:60px;position:relative;top:10px;left:76%;"><button style="border:none;background:#f9f9f9;color:#988312;font-size:13px;">'+title2+'</button></div>';
		html += '</div>';
		html += '</div>';
		html += '</div>';
	}
//	
	...
	html += getButton(data,data.id,parseInt(data.status),false,parseInt(data.groupStatus),data.orderDetails);
	return html;
}
...
function openpay(orderId) {
	...
	$(".CouponBulletinSeven").toggleClass("CouponBulletin-on", 1000);
...
function successpay(ret, err) {
	...
	if (ret.code == 9000) {
		var jsonData = {};
		jsonData.orderId = orderData.id;
		jsonData.orderType = orderData.orderType;
