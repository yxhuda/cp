...
var hui = (function(selector){
	...
	var hcExtends = {
		...
		scrollX : function(num, sonsTag, extraValue){
			if(this.length < 1){return} if(!num){num = 3;} if(!sonsTag){sonsTag = 'div';} if(!extraValue){extraValue = 0;}
			...
			for(var i = 0; i < this.length; i++){
				...
				sons.css({'margin':(setWitdh+extraValue)+'px', 'margin':'10px 5px'});
				//cObj.find('div').eq(0).css({width : (setWitdh * total) + ''});zp修改，关于加边距元素换行
	...
	/* count down */
	_hui.countdown = function(timer, domId, showType,backClick){
		if(!showType){showType = 1;}
		hui.countdownBase(timer, domId, showType);
		
		setInterval(function(){
	
			hui.countdownBase(timer, domId, showType,backClick);	
		}, 1000);
	}
	_hui.countdownBase = function(timer, domId, showType,backClick){
